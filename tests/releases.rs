...
async fn test_update_release_for_product_without_name() -> Result<(), Box<dyn std::error::Error>> {
    let m = mock("PUT", "/api/v1/products/PROD-1/releases/REL-1")
        .match_header("Authorization", BEARER_TOKEN)
        .match_body(Matcher::Json(json!({"release":{"parent_id":"PROD-2"}})))
        .with_status(204)
        .create();

    let uri = &mockito::server_url();
    let aha = AhaRequest::with_url(TEST_TOKEN, TEST_SUBDOMAIN, uri);
    aha.update_release_for_product("PROD-1", "REL-1", &None, &Some("PROD-2".to_string()))
        .await?;
...
#[async_std::test]
async fn test_update_release_for_product_without_parent_id(
    ...
    let uri = &mockito::server_url();
    let aha = AhaRequest::with_url(TEST_TOKEN, TEST_SUBDOMAIN, uri);
    aha.update_release_for_product("PROD-1", "REL-1", &Some("newname".to_string()), &None)
        .await?;
